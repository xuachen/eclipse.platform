...
public class FeatureInstaller {
	...
	public FeatureInstaller(FeatureInstaller parentInstaller,
			InstalledSite targetSite, IFeature sourceFeature) {
		this.parentInstaller = parentInstaller;
		this.targetSite = targetSite;
		this.sourceFeature = sourceFeature;
	}

	public IFeature install(IFeature[] optionalfeatures,
			IVerifier parentVerifier,
			IVerificationListener verificationListener, InstallMonitor monitor) throws InstallAbortedException, CoreException {
		...
		InstallHandlerProxy handler = new InstallHandlerProxy(
				IInstallHandler.HANDLER_ACTION_INSTALL, sourceFeature,
				sourceFeature.getInstallHandler(), (InstallMonitor) monitor);
		...
		IFeatureContentProvider provider = sourceFeature
		...
		try {
			...
			// find the intersection between the plugin entries already
			// contained
			// on the target site, and plugin entries packaged in source
			// feature
			IPluginEntry[] sourcePlugins = sourceFeature.getPluginEntries(true);
			IPluginEntry[] targetPlugins = targetSite.getPluginEntries();
			IPluginEntry[] pluginsToInstall = UpdateManagerUtils.diff(
					sourcePlugins, targetPlugins);

			INonPluginEntry[] nonPluginsToInstall = sourceFeature
					.getNonPluginEntries(true);

			IFeature[] children = sourceFeature.getIncludedFeatures(true);
			if (optionalfeatures != null) {
				children = UpdateManagerUtils.optionalChildrenToInstall(
						children, optionalfeatures);
			}

			// determine number of monitor tasks
			//   2 tasks for the feature jar (download/verify + install)
			// + 2*n tasks for plugin entries (download/verify + install for
			// each)
			// + 1*m tasks per non-plugin data entry (download for each)
			// + 1 task for custom non-plugin entry handling (1 for all
			// combined)
			// + 5*x tasks for children features (5 subtasks per install)
			int taskCount = 2 + 2 * pluginsToInstall.length
					+ nonPluginsToInstall.length + 1 + 5 * children.length;
			...
			for (int i = 0; i < children.length; i++) {
				...
				FeatureInstaller childInstaller = new FeatureInstaller(this,
						targetSite, childFeature);
				featureInstallers.add(childInstaller);
				childInstaller.install(optionalfeatures, verifier,
						verificationListener, new InstallMonitor(subMonitor));
			}

			// Install plugin files
			for (int i = 0; i < pluginsToInstall.length; i++) {
				// if another feature has already installed this plugin, skip
				// it
				if (InstallRegistry.getInstance()
						.isPluginJustInstalled(pluginsToInstall[i])) {
					monitor.worked(1);
					continue;
				}
				PluginInstaller pluginInstaller = new PluginInstaller();
				pluginInstallers.add(pluginInstaller);
				pluginInstaller.install(pluginsToInstall[i], sourceFeature,
						targetSite, monitor);
			}
			...
			targetFeature = targetSite.getFeature(sourceFeature
					.getVersionedIdentifier(), null);
			// 18867
			...
			try {
				...
				if (success) {
					...
					if (targetFeature == null) {
						// Create the feature out of the newly installed files
						try {
							targetFeature = targetSite.getFeature(new File(
									getFeaturePath())
									.toURL(),
									null);
						...
						if (optionalfeatures != null && optionalfeatures.length > 0)
			...
			if (originalException != null) {
				throw Utilities.newCoreException(Policy.bind(
						"InstallHandler.error", sourceFeature.getName()),
						originalException);
			}

			if (newException != null)
				throw Utilities.newCoreException(Policy.bind(
						"InstallHandler.error", sourceFeature.getName()), newException);
		...
		// TODO need to refresh the targetfeature and the installed site one
		// installation is complete
		return targetFeature;
	}

	private void verifyReferences(IVerifier verifier,
			ContentReference[] references, InstallMonitor monitor,
			IVerificationListener verificationListener, boolean isFeature) throws CoreException {
		IVerificationResult vr = null;
		if (verifier != null) {
			for (int j = 0; j < references.length; j++) {
				vr = verifier.verify(sourceFeature, references[j], isFeature,
						monitor);
				if (vr != null) {
					...
					if (result == IVerificationListener.CHOICE_ABORT) {
						String msg = Policy
								.bind("JarVerificationService.CancelInstall"); //$NON-NLS-1$
					...
					if (result == IVerificationListener.CHOICE_ERROR) {
						throw Utilities
								.newCoreException(
										Policy
												.bind("JarVerificationService.UnsucessfulVerification"), 
										//$NON-NLS-1$
	...
	private String getFeaturePath() throws CoreException {
		...
		try {
			VersionedIdentifier featureIdentifier = sourceFeature
					.getVersionedIdentifier();
			String path = Site.DEFAULT_INSTALLED_FEATURE_PATH
					+ featureIdentifier.toString() + File.separator;
			URL newURL = new URL(targetSite.getURL(), path);
			featurePath = newURL.getFile();
		} catch (MalformedURLException e) {
			throw Utilities.newCoreException(Policy
					.bind("SiteFileContentConsumer.UnableToCreateURL")
					+ e.getMessage(), e);
			//$NON-NLS-1$
		}
		return featurePath;
	...
	private void storeFeature(IFeature sourceFeature,
			ContentReference contentReference, IProgressMonitor monitor) throws CoreException {

		if (installComplete) {
			UpdateCore.warn(
					"Attempt to store in a closed SiteFileContentConsumer",
					new Exception());
			return;
		}
		...
		if (featurePath.endsWith("\\" + Feature.FEATURE_XML)
				|| featurePath.endsWith("/" + Feature.FEATURE_XML)) {
			oldPath = featurePath.replace(File.separatorChar, '/');
			File localFile = new File(oldPath);
			if (localFile.exists()) {
				throw Utilities.newCoreException(Policy.bind(
						"UpdateManagerUtils.FileAlreadyExists",
						new Object[]{localFile}), null);
			}
			featurePath = ErrorRecoveryLog
					.getLocalRandomIdentifier(featurePath);
			newPath = featurePath;
			ErrorRecoveryLog.getLog()
					.appendPath(ErrorRecoveryLog.FEATURE_ENTRY, featurePath);
		}
		...
		try {
			...
			throw Utilities.newCoreException(Policy.bind(
					"GlobalConsumer.ErrorCreatingFile", featurePath), e);
			//$NON-NLS-1$
		} finally {
			if (inStream != null) {
				try {
	...
	public void completeInstall() throws CoreException {
		...
		if (installComplete)
			UpdateCore.warn(
					"Attempt to close a closed SiteFileContentConsumer",
					new Exception());

		//rename featurexxxxxx.xml file back to feature.xml
		if (newPath != null) {
			ErrorRecoveryLog.getLog()
					.appendPath(ErrorRecoveryLog.RENAME_ENTRY, newPath);
			...
			if (fileToRename.exists()) {
				...
				if (renamedFile.exists()) {
					...
					UpdateCore
							.warn("Removing already existing file:" + oldPath);
				}
				sucess = fileToRename.renameTo(renamedFile);
			}
			if (!sucess) {
				String msg = Policy.bind("ContentConsumer.UnableToRename",
						newPath, oldPath);
		...
		for (int i = 0; i < pluginInstallers.size(); i++) {
			PluginInstaller pluginInstaller = (PluginInstaller) pluginInstallers
					.get(i);
		...
		for (int i = 0; i < featureInstallers.size(); i++) {
			FeatureInstaller featureInstaller = (FeatureInstaller) featureInstallers
					.get(i);
