...
public class FeatureParser extends DefaultHandler {
			...
			case STATE_HANDLER :
				...
				if (objectStack.peek() instanceof InstallHandlerEntry) {
					...
					//$NON-NLS-1$
						...
						featureModel.setInstallHandler(handlerModel);
				}
				break;
						...
						case STATE_FEATURE :
							if (objectStack.peek() instanceof Feature) {
								...
								//$NON-NLS-1$
									...
									featureModel.setDescription(info);
							}
							break;
						...
						case STATE_FEATURE :
							if (objectStack.peek() instanceof Feature) {
								...
								//$NON-NLS-1$
									...
									featureModel.setCopyright(info);
							}
							break;
	...
	private void processHandler(Attributes attributes) {
		InstallHandlerEntry handler = new InstallHandlerEntry();
	...
	private void processPlugin(Attributes attributes) {
		...
		|| ver == null || ver.trim().equals("")) { //$NON-NLS-1$
			...
			// download size
			long download_size = ContentEntry.UNKNOWN_SIZE;
			...
			// install size	
			long install_size = ContentEntry.UNKNOWN_SIZE;
	...
	private void processData(Attributes attributes) {
		...
		if (id == null || id.trim().equals("")) { //$NON-NLS-1$
			...
			NonPluginEntry dataEntry = new NonPluginEntry();
